...
objconexion=DriverManager.getConnection("jdbc:mysql://localhost:3306/proyecto",
                                        "root","mysql");//ojo MAYUSCULAS y Minusculas
...
try{
	// necesitamos la libreria  mysql-connector-java-8.0.28-bin coon los drives y objetos jdbc 
	Class.forName("com.mysql.cj.jdbc.Driver");

objconexion=DriverManager.getConnection("jdbc:mysql://localhost:3306/proyecto","root","mysql");//ojo MAYUSCULAS y Minusculas

}
catch(ClassNotFoundException e){
	throw new DAOException("no se ha cargado los controladores jdbc "+ e.getMessage(),e);
}
catch(SQLException e){
	throw new DAOException("no se ha establecido la conexion a la bbdd",e);
}												 
